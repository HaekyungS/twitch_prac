...
class MyClient(discord.Client):
    async def on_ready(self):
        ...
        while True:
            ...
            # 'https://api.twitch.tv/helix/streams?user_login=onyu74', headers=headers)
            print(response_channel.text)
            # 라이브 상태 체크
            try:
                # 방송 정보에서 'data'에서 'type' 값이 live 이고 체크상태가 false 이면 방송 알림(오프라인이면 방송정보가 공백으로 옴)
                if (
                    loads(response_channel.text)["data"][0]["type"] == "live"
                    and check == False
                ):
                ...
                # await channel_bangOn.send('아카뱅 종료')
                # await channel_test.send('아카뱅 종료')
                print("Offline")
                check = False
            # 30초마다 요청을 보낸다.
    ...
    async def on_message(self, message):
        ...
        if message.content in ("안녕", "안녕하세요", "하이", "hi", "hello", "안뇽"):
            await message.channel.send(f"{message.author.mention} 안녕! 너도 선타대야?")
        if (
            any(word in message.content for word in AkaName)
            and "귀여워" in message.content
        ):
            await message.channel.send("아니? 나 안귀여운데. 나 멋있는데! 니가 몰 알아")
        ...
        if message.content == "좋아해":
            await message.channel.send("난 너 안좋아하는데? 사랑하는데")
